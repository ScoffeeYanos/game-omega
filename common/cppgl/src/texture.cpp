...
Texture2DImpl::Texture2DImpl(const std::string &name, const fs::path &path, bool mipmap, bool flipped) : name(name), loaded_from_path(path), id(0)
{
    // load image from disk
    auto [data, w_out, h_out, channels, is_hdr] = image_load(path, flipped);
    this->w = w_out;
    this->h = h_out;
    ...
    : name(name), id(0), w(w), h(h), internal_format(internal_format), format(format), type(type)
{
    ...
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER,
                    mipmap ? GL_LINEAR_MIPMAP_LINEAR : (format == GL_DEPTH_COMPONENT || format == GL_DEPTH_STENCIL) ? GL_NEAREST
                                                                                                                    : GL_LINEAR);
...
{
    // if layered (4) is GL_TRUE, the entire array is attached to the image unit and layer (5) is ignored.
    glBindImageTexture(unit, id, 0, GL_TRUE, 0, access, format);
}

void Texture3DImpl::unbind_image(uint32_t unit) const
{
    glBindImageTexture(unit, 0, 0, GL_TRUE, 0, GL_READ_ONLY, GL_RGBA8);
}
